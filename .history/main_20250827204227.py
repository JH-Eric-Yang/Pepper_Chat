...
class NAOqiBridge:
    ...
    def speak(self, text: str, language: str = "English") -> bool:
        ...
        # Test audio system first if not already done
        if not hasattr(self, '_audio_tested'):
            if self.test_audio():
                self._audio_tested = True
                time.sleep(2)  # Wait after audio test like in test_bridge.py
            else:
                return False
...
class VoiceInteractionSystem:
    ...
    def wait_for_speech_detection(self) -> Optional[str]:
        """Wait for speech detection with improved logic"""
                        ...
                        if user_text:
                            ...
                            if self.handle_system_commands(user_text):
                                ...
                                if robot_response:
                                    ...
                                    if speech_result:
                                        print("Robot finished speaking" + speech_result)
                                    else:
